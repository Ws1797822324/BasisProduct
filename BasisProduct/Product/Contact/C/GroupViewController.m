...
- (UIView *)addHeadView {
    if(_addHeadView == nil) {
        _addHeadView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 60)];
        ...
        UIView *view1 = [[UIView alloc]initWithFrame:CGRectMake(0, 60 - 0.5, kScreenWidth, 0.5)];
        view1.backgroundColor = kRGB_HEX(0xC8C7CC);
        [_addHeadView addSubview:view1];
        UIButton *addBtn = [UIButton buttonWithType:UIButtonTypeSystem];
        addBtn.frame = CGRectMake(0, 0, kScreenWidth, 60);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row != 0) {
        cell.textLabel.textColor = kRGB_HEX(0x363636);
        cell.textLabel.font = [UIFont systemFontOfSize:16];
        GroupModel *model = _groupModelArr[indexPath.row];
...
#pragma mark- 点击 删除 按钮的操作
    ...
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        GroupModel *dModel = _groupModelArr[indexPath.row];
